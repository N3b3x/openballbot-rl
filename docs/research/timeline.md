...
- The three-omniwheel configuration in `ballbot_gym/assets/bbot.xml` matches this design exactly
...
- The policy gradient updates in `ballbot_rl/training/train.py` use PPO's clipped objective
...
- `ballbot_rl/training/train.py` uses Stable-Baselines3's PPO implementation
...
- `ballbot_gym/bbot_env.py` is the Gymnasium environment from the paper
- `ballbot_rl/training/train.py` trains PPO policies as described
- `ballbot_gym/terrain.py` generates Perlin noise terrain
- `outputs/encoders/encoder_epoch_53` is the pretrained depth encoder
- `configs/train_ppo_directional.yaml` matches the hyperparameters
...
| **Schulman 2017** | PPO algorithm (clipped objective) | `ballbot_rl/training/train.py` (PPO training, clip_range) |
| **Raffin 2021** | Stable-Baselines3 implementation | `ballbot_rl/training/train.py` (training infrastructure) |
