...
The training configuration is in `configs/train_ppo_directional.yaml`:
...
The `ballbot_rl/training/train.py` script:
...
```bash
cd scripts
python3 train.py --config ../configs/train_ppo_directional.yaml
```
...
# ... run episodes ...
# GIFs saved to outputs/visualizations/images/ directory
```
