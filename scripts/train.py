...
def main(config, seed):
    ...
    policy_kwargs = dict(
        ...
        features_extractor_class=policies.
        Extractor,  #note that this will be shared by the policy and the value networks
    ...
    if os.path.exists(config['out']):
        if confirm(
                colored(
                    f"The output directory ({config['out']}) specified in your yaml file already exists. Overwrite?",
                    "red",
                    attrs=["bold"])):
